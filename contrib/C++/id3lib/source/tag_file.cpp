...
static int truncate_file(const char *path, size_t length)
{
  int result = -1;
  HANDLE fh;
...
flags_t ID3_TagImpl::Strip(flags_t ulTagFlag)
{
  ...
  if (ulTagFlag & ID3TT_PREPENDED & _file_tags.get())
  {
    ...
    // copied the data, we'll truncate_file the file.
  ...
  if (ulTags && (truncate_file(_file_name.c_str(), nNewFileSize) == -1))
  {
    // log this
    return 0;
